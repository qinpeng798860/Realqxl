...
$(function(){
    ...
    if ($("#rCbm").val()!= ''){
        $("#myGeenCheckLabel").html("绿化检查修改");
        url = rootPath+"check/updateGeenCheck?";//必须加?号
    }else{
        $("#myGeenCheckLabel").html("新增绿化检查植被");
        url = rootPath+"check/insertGeenCheck?"

    }

    $("#editaddGeenCheckForm").bootstrapValidator({
    ...
    /*提交表单*/
    $("#addGeenCheckBtn_submit").click(function(){
        //业务逻辑
        editaddGeenCheckSubmit();
    })
})
// JSON.stringify(option)
function editaddGeenCheckSubmit(){
    //获取验证结果
    var bootstrapValidator = $("#editaddGeenCheckForm").data('bootstrapValidator');
    //手动开启验证
    bootstrapValidator.validate();
    //查看验证是否通过
    if(bootstrapValidator.isValid()){
        var option=$("#editaddGeenCheckForm").serialize();//序列化获取表单数据
       ...
       // console.log(option+"-+++++++++++-");option
        $.ajax({
            ...
            success:function(data){
                console.log(data+"--");
                alert(data.message);
                $("#addGeenCheckModal").modal('hide');
            }
        });
        $("#editaddGeenCheckForm").data('bootstrapValidator').destroy();//验证清除
        $("#editaddGeenCheckForm")[0].reset();//表单重置
    }
}
