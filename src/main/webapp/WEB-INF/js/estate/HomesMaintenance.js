...
function initTypeServerTable() {
    $('#typeServerTable').bootstrapTable({
        url:'estate/queryListResidential',         //请求后台的URL（*）
        method: 'post',                      //请求方式（*）
        ...
        columns: [{
            ...
            width:'15%', formatter:function(value,row,index){
                ...
                var btn="<a class='link-button' href='javascript:updateTypeServer(\""+bh+"\")'>bh</a>";
                return btn;
            }
        }, {
            field: 'rZzdz',
            ...
            formatter:function(value,row,index){
                ...
                var btn="<a class='link-button' href='javascript:queryListBuilding(\""+bh+"\")'>查看楼宇 </a>";
                return btn;
            }
        }, ]
    });
...
function queryList(){
    ...
    var opt={
        url: "estate/queryListResidential"
    }
...
function addTypeServer(){
    $("#TypeServerModal").modal({
        remote:rootPath+"/estate/addHomes.shtml",
...
function DelLy(){
    var option=$('#typeServerTable').bootstrapTable('getSelections');//获取的是数组
    var a = option[0].rZzbh;
    console.log(option)
    if(option!=''){
        $.ajax({
            url:rootPath+'estate/delResidential',
            data:{"zzbh":a},
            dataType:"json",
            async:false,
            type:"post",
...
function queryListBuilding(bh){
    console.log(bh);

    window.location.href="building/queryListBuilding.shtml?"+"zzbh="+bh;
}
